...
option_list = list(
  ...
  make_option(c("--Not_only_pos"), action="store_false", default = TRUE,
...
if (opt$DGE_Clusters_vs_all) {
	...
	pdf(paste(opt$outDir,opt$Sample,'/FindMarkers_vs_all/DGE_Markers_top',opt$Top_n,'_heatmap.pdf',sep=''),width = 3000, height = 1500)
	print(heat_map)
	dev.off()
	...
	for (cluster in clusters) {
	  ...
	  if ('Group' %in% colnames(df))
		df = df[df$Group=='Up',]
	  df = df[df$pct.1>=opt$min_pct1 & df$p_val_adj<opt$ALPHA,]
	  ...
	  if (dim(df)[1]>0) {
		...
		pdf(paste(opt$outDir,opt$Sample,'/FindMarkers_vs_all/Cluster',cluster,'_heatmap_vs_all.pdf',sep=''),width = 3000, height = 1500)
		print(heat_map)
		dev.off()
	  }
	}
		...
		if (DGE_GroupBy %in% colnames(obj_seurat@meta.data)){
			...
			for (cluster in clusters){
				...
				if (any(opt$Force,!file.exists(file))) {
				  ...
				  # DGE_Markers <- FindMarkers(obj_seurat, ident.1 = ident1, subset.ident = cluster ,group.by = DGE_GroupBy, only.pos = opt$Not_only_pos, min.pct = 0.1, 
				...
				pdf(paste(dir_path,'DGE_',ident1,'_Within_Cluster_',cluster,'_top',opt$Top_n,'_heatmap.pdf',sep=''),width = 15, height = 30)
				print(heat_map)
				dev.off()
				...
				if (dim(df)[1]>0) {
					...
					pdf(paste(dir_path,'DGE_',ident1,' Within Cluster ',cluster,'_heatmap.pdf',sep=''),width = 15, height = 30)
					print(heat_map)
					dev.off()
				}
...
if (opt$DGE_Pairwise) {
	...
	dir_path = paste(opt$outDir,opt$Sample,'/FindMarkers_vs_each_other/',ident1,'_vs_',ident2,'/',sep='')
	if (!dir.exists(dir_path))
		dir.create(dir_path)
	...
	write.csv(DGE_Top_Markers, paste(dir_path,'DGE_',ident1,'_vs_',ident2,'_Markers_top',opt$Top_n,'.csv',sep=''))
	writeLog(logfile, paste("Plotting top ",opt$Top_n," marker genes",sep=''))
	if (!is.na(opt$Subsample)) {
	  heat_map = DoHeatmap(subset(obj_seurat, downsample=subsample), features = DGE_Top_Markers$gene) + NoLegend()
	} else {
	...
	pdf(paste(dir_path,'DGE_',ident1,'_vs_',ident2,'_top',opt$Top_n,'_heatmap.pdf',sep=''),width = 3000, height = 1500)
	print(heat_map)
	dev.off()
	...
	if ('Group' %in% colnames(df))
		df = df[df$Group=='Up',]
	df = df[df$pct.1>=opt$min_pct1 & df$p_val_adj<opt$ALPHA,]
	writeLog(logfile, paste("Contrast ",ident1,' vs. ',ident2,': ',dim(df)[1]," Up-regulated Differentially Expressed Genes",sep=''))
	if (dim(df)[1]>0) {
		# Significant Genes Heatmap
		sig_genes <- df$gene
		if (!is.na(opt$Subsample)) {
		...
		pdf(paste(dir_path,'DGE_',ident1,'_vs_',ident2,'_Up_Regulated_heatmap.pdf',sep=''),width = 3000, height = 1500)
		print(heat_map)
		dev.off()
		...
		if (!inherits(enrichGO_compClusters,"try-error")) {
			enrichGO_compClusters@compareClusterResult$GeneRatio=paste("'",enrichGO_compClusters@compareClusterResult$GeneRatio,sep='')
			write.csv(enrichGO_compClusters@compareClusterResult,
					  file=paste(dir_path,'DGE_',ident1,'_vs_',ident2,'_GO_compareClusters.csv',sep=''),quote = FALSE,row.names = TRUE)
			enrichGO_simplify <- try(clusterProfiler::simplify(enrichGO_compClusters, cutoff = opt$simplify_cutoff),
									 silent = TRUE)
			if (!inherits(enrichGO_simplify,"try-error")) {
			  enrichGO_simplify@compareClusterResult$GeneRatio=paste("'",enrichGO_simplify@compareClusterResult$GeneRatio,sep='')
			  write.csv(enrichGO_simplify@compareClusterResult,
						file=paste(dir_path,'DGE_',ident1,'_vs_',ident2,'_GO_simplify.csv',sep=''),quote = FALSE,row.names = TRUE)
			  ...
			  enrichGO_simplify@compareClusterResult$GeneRatio=stringr::str_remove_all(enrichGO_simplify@compareClusterResult$GeneRatio,"'")
			  enrichGO_dotplot <- enrichplot::dotplot(enrichGO_simplify, showCategory=10000, font.size=fontsize, label_format=100)
			  ggplot2::ggsave(filename = paste(dir_path,'DGE_',ident1,'_vs_',ident2,'_GO_simplify_dotplot.pdf',sep=''),
							  enrichGO_dotplot, dpi = 600,device = "pdf",width = 30,height = 50, limitsize = FALSE)
			}
		}
	}
