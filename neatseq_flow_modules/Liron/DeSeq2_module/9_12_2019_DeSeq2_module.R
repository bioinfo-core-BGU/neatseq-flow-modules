...
create_excel_output <- function(dds, contrusts, Norm_Type, output_path, dds_lrt, fixed_norm_counts, linear_fc_cutoff, post_linear_fc_cutoff, alpha_cutoff, post_alpha){
...
if (opt$only_clustering==FALSE){
  ...
  if (is.na(opt$LRT)){
    create_excel_output(DESeqDataSet_base, test_count, opt$NORMALIZATION_TYPE ,paste0(opt$outDir, "summary_table"), linear_fc_cutoff=opt$FoldChange, post_linear_fc_cutoff=opt$Post_statistical_FoldChange, alpha_cutoff=opt$ALPHA, post_alpha=opt$Post_statistical_ALPHA)
    ...
    create_excel_output(DESeqDataSet_base, test_count, opt$NORMALIZATION_TYPE ,paste0(opt$outDir, "summary_table"), DESeqDataSet_redued, linear_fc_cutoff=opt$FoldChange, post_linear_fc_cutoff=opt$Post_statistical_FoldChange, alpha_cutoff=opt$ALPHA, post_alpha=opt$Post_statistical_ALPHA)
...
if (!is.na(DESeqDataSet)){
  ...
  if (!is.na(opt$removeBatchEffect)){
    ...
    if (is.na(opt$LRT)){
      create_excel_output(DESeqDataSet, test_count, opt$NORMALIZATION_TYPE ,paste0(opt$outDir, "summary_table_batch_effect"), linear_fc_cutoff=opt$FoldChange, post_linear_fc_cutoff=opt$Post_statistical_FoldChange, alpha_cutoff=opt$ALPHA, post_alpha=opt$Post_statistical_ALPHA)
      genral_test_name <- "GENERAL"
    }
    else {
      ...
      create_excel_output(DESeqDataSet, test_count[-1], opt$NORMALIZATION_TYPE ,paste0(opt$outDir, "summary_table_batch_effect"), DESeqDataSet_redued, linear_fc_cutoff=opt$FoldChange, post_linear_fc_cutoff=opt$Post_statistical_FoldChange, alpha_cutoff=opt$ALPHA, post_alpha=opt$Post_statistical_ALPHA)
    }
...
for (test2do in test_count){
  print(test2do)
  if (Rmarkdown_flag){
    if ((test2do=='LRT')||(test2do=='No_DESIGN')||(test2do=='only_clustering') ){
      test_name=test2do
    }else{
        ...
        for (SPLIT in SPLIT_BY){ 
          ...
          if (sum(duplicated(SPLIT_Normalized_counts_Annotated_mean)==F)>1){
            if (nrow(SPLIT_Normalized_counts_Annotated_mean)>2) {
              ...
              print('Since only 2 significant genes were detected, No clustering was performed')
