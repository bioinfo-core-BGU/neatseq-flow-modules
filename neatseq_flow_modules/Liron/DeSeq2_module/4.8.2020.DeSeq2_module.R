    ...
    if (is.na(Annotation)) {
      if (!is.na(opt$Species)){
        ...
        if ("package:biomaRt" %in% search()){
          dataset=c()
          try_count=0
          while ((length(dataset) == 0)&&(try_count<3) ){
            try_count = try_count+1
            mirrors = c('useast','uswest','asia','www')
            biomart = "genes"
            M <- biomaRt::useEnsembl(biomart = biomart,mirror=mirrors[try_count])
            ...
            if (length(dataset) == 0){
              ...
              biomart = 'plants_mart'
              # M=biomaRt::useMart(biomart = biomart,host = host,ensemblRedirect = FALSE)
              M = useEnsemblGenomes(biomart = biomart ,mirror=mirrors[try_count])
            ...
            if (biomart=='plants_mart'){
                ensembl <- try(biomaRt::useEnsemblGenomes(biomart=biomart, dataset=as.character(dataset),mirror = mirrors[try_count]),silent = T)
            }else{
                ensembl <- try(biomaRt::useEnsembl(biomart=biomart, dataset=as.character(dataset),mirror = mirrors[try_count]),silent = T)
            }
            while ((inherits(ensembl,"try-error")) &&( try_count2<5) ){
          ...
          if (length(dataset)==1){
            ...
            if ( opt$GENE_ID_TYPE %in% listAttributes(ensembl)$name){
              ...
              for (subset_genes in split(genes[,opt$GENE_ID_TYPE], ceiling(seq_along(genes[,opt$GENE_ID_TYPE] )/1000))){
                try_count3=0
                try_count3_limit=4
                while (try_count3<try_count3_limit){
                    ...
                    if (inherits(Annotation,"try-error")){
                        print('biomart error')
                        try_count3=try_count3+1
                    }else{
                        try_count3=try_count3_limit
