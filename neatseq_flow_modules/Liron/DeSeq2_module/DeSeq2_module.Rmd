...
clusters_enricher=function(outDir, clusters,Type,organism,universe,keyType,file_name,Ontology="BP",pvalueCutoff=0.05,pAdjustMethod='fdr',qvalueCutoff=0.05,maxCategory=1000){
    ...
    if (dim(allRes@compareClusterResult)[1]>0){
        ...
        if (dim(allRes@compareClusterResult)[1]>0){
            ...
            x=enrichplot::dotplot(allRes,x=allRes@compareClusterResult$Cluster,showCategory=maxCategory,font.size =font.size)+
        ...
        if (! general){
            ...
            if (!is.na(opt$significant_genes)){ 
                ...
                if (dim(sig.genes)[2]==1){
                    ...
                    Data= sapply(X =  as.list(Data),FUN = na.omit)
                    clusters = c()
                    for (cluster in names(Data)){
                      Temp_Data = intersect(Data[[cluster]],row.names(Normalized_counts_assay))
                      TEMP_clusters = rep(cluster,length(Temp_Data) )
                      names(TEMP_clusters) = Temp_Data
                      clusters = c(clusters,TEMP_clusters)
                    }
                }
