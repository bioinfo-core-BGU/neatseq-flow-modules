...
getDoParWorkers()
    ...
    if (!is.na(opt$PATHWAYS_2_USE)){
        pathways = unlist(stringi::stri_split(str =  opt$PATHWAYS_2_USE,regex = ','))
    }
    
    for (pathway in pathways){
      print(pathway)
      pathway.id = stringi::stri_replace(str = pathway,fixed = 'path:map',replacement = '')
      ...
      if (nrow(pathway_data)>0){ 
          ...
          if (nrow(pathway_data)>0){ 
            ...
            Graph_data_frame      = foreach(i=1:length(pivot_table_colnames) ,.combine=cbind) %dopar% {
                                    file_name           = Cooperation_data_Total[i]
                                    print(file_name)
                                    load(file_name)
                                    I = pivot_table_colnames[i]
                                    ...
                                    for (j in pivot_table_colnames){ 
                                        ...
                                        if (I==j){
                                              ...
                                              #Cooperation_index_matrix[i,j]      = Cooperation_index
                                              Graph_data[j,I]          = Create_Graph_metabolic(merge_path,pathway_data,original_path,overlap_edges)
                                              #Overlap_data_frame[i,j]            = overlap
                                              #original_data_frame[i,j]           = nrow(original_path)
            ...
            New_Edges_data_frame = foreach(i=1:length(pivot_table_colnames) ,.combine=cbind) %dopar% {
                                    file_name           = Cooperation_data_Total[i]
                                    print(file_name)
                                    load(file_name)
                                    I = pivot_table_colnames[i]
                                    ...
                                    for (j in pivot_table_colnames){ 
                                        ...
                                        if (I==j){
                                              #Cooperation_index_matrix[i,j]      = NA
                                              New_Edges_data[j,I]          = NA
                                              #Overlap_data_frame[i,j]            = NA
                                              #original_data_frame[i,j]           = NA
                                        }else{
                                              #Cooperation_index_matrix[i,j]      = Cooperation_index
                                              New_Edges_data[j,I]          = new_edges
                                              #Overlap_data_frame[i,j]            = overlap
                                              #original_data_frame[i,j]           = nrow(original_path)
            ...
            Overlap_data_frame = foreach(i=1:length(pivot_table_colnames) ,.combine=cbind) %dopar% {
                                    file_name           = Cooperation_data_Total[i]
                                    print(file_name)
                                    load(file_name)
                                    I = pivot_table_colnames[i]
            ...
            Original_data_frame = foreach(i=1:length(pivot_table_colnames) ,.combine=cbind) %dopar% {
                                    file_name           = Cooperation_data_Total[i]
                                    print(file_name)
                                    load(file_name)
                                    I = pivot_table_colnames[i]
                                    ...
                                    for (j in pivot_table_colnames){ 
                                        ...
                                        if (I==j){
                                              ...
                                              #Cooperation_index_matrix[i,j]      = Cooperation_index
                                              New_data[j,I]          = dim(original_path)[1]
                                              #Overlap_data_frame[i,j]            = overlap
                                              #original_data_frame[i,j]           = nrow(original_path)
            ...
            write.table(x = Original_data_frame,
